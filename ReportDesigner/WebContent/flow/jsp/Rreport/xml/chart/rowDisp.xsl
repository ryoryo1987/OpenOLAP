...
<xsl:variable name="maxRow" select="count(//data/row)"></xsl:variable>
...
<xsl:template match="data">
	...
	<xsl:call-template name="loop">
		...
		<xsl:with-param name="rowVal" select="/data/row[number(//OpenOLAP/property/dispRow/@*[name()='startRow'])]" />
	</xsl:call-template>
</xsl:template>
...
<xsl:template name="loop">
	...
	<xsl:if test="$count &lt;= $end">
		...
		<xsl:call-template name="loop">
			...
			<xsl:with-param name="rowVal" select="/data/row[$count+1]" /> 
		</xsl:call-template>
	</xsl:if>
</xsl:template>
<!--*******************************************************************************-->
