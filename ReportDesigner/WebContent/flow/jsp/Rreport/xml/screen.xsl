...
//alert(source.xml);
//	xmlData.loadXML(source.xml);

//	var strResult = xmlData.transformNode(cssXSL);
//	var treeObj = parent.frm_css.document.getElementById("treeObj");
//	treeObj.innerHTML = strResult;
	parent.frm_css.loadCssXML(source.xml);

	//Set Property
	var screenType = parent.frm_property.document.getElementById("screenType");
	screenType.innerText=source.getAttribute("name");
	screenType.seq=source.getAttribute("id");

	var styleType = parent.frm_property.document.getElementById("styleType");
	styleType.innerText="未選択";
	styleType.seq="";

	//hidden obj
	var hidObj= parent.parent.frm_backNext.document.getElementById("screen_id");
	hidObj.value='';
}
function getScreenKindXML(){
...
<table class="Header">
	<tr>
		...
		<td class="HeaderTitleCenter">テンプレート</td>
	</tr>
</table>
	...
	<xsl:element name="div">
		...
		<xsl:element name="a">
			...
			<xsl:attribute name="onclick">
				<xsl:choose>
					<xsl:when test="local-name()!='folder' and local-name()!='folder2'">
...
<xsl:template name="tree-art-hierarchy">
    <xsl:for-each select="ancestor::*">
        <xsl:choose>
			<xsl:when test="local-name()='screens'"> </xsl:when>
