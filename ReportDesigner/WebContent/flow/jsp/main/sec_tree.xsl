...
<script language="JavaScript1.2" src="sec_load.js"></script>
...
<!--	<xsl:attribute name="onload"><xsl:text>javaScript:Toggle('r',document.getElementById('root').firstChild.firstChild,'sec_list.jsp?id=root')</xsl:text></xsl:attribute>-->
...
<xsl:template match="/" mode="tree-art">
	<div id="root" class="treeItem">
		<xsl:element name="img">
			...
			<xsl:attribute name="onclick">javascript:Toggle('r',this,'sec_list.jsp?id=root');</xsl:attribute>
			<xsl:attribute name="objkind"><xsl:value-of select="/*/@OBJKIND"/></xsl:attribute>
		</xsl:element>

		<xsl:element name="a">
			<xsl:attribute name="href">return false;</xsl:attribute>
			<xsl:attribute name="onclick">javascript:Toggle('r',this,'sec_list.jsp?id=root'); return false;</xsl:attribute>
			<xsl:if test="//BistroObjects[./@TYPE!='1' or ./@PARTS!='1']">
	...
	<xsl:if test="@KI='F'">
		...
		<xsl:variable name="imgFileName">
			...
			<!--	<xsl:when test="(local-name()='category') and @KI='F'">../../../images/tree/folder2.gif</xsl:when>-->
					<xsl:when test="(local-name()='category') and @KI='F'">
							<xsl:choose>
								<xsl:when test="@RIGHT='1'">
									../../../images/tree/folder.gif
								</xsl:when>
								<xsl:otherwise>
									../../../images/tree/folder_no.gif
								</xsl:otherwise>
							</xsl:choose>
					</xsl:when>
		...
		<xsl:variable name="execFileName">
			<xsl:choose>
				...
				<xsl:otherwise>sec_list.jsp</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<xsl:variable name="dispStyle">
			<xsl:choose>
				...
				<xsl:when test="@disp='block'">display:block;</xsl:when>
...
<xsl:template name="tree-art-hierarchy">
    <xsl:for-each select="ancestor::*">
        <xsl:choose>
			...
			<xsl:when test="following-sibling::*[@KI='F']"><img id="H" src="../../../images/tree/I.gif"/></xsl:when>
   ...
   <xsl:choose>                                                                                                      
		...
		<xsl:when test = "child::*[@KI='F'] and not(following-sibling::*[@KI='F'])">
			<img id="5" src="../../../images/tree/Lplus.gif" onclick="JavaScript:Toggle('LP',this,'');"/>
		</xsl:when>

		<xsl:when test = "child::*[@KI='F'] and following-sibling::*[@KI='F']">
			<img id="5" src="../../../images/tree/Tplus.gif" onclick="JavaScript:Toggle('TP',this,'');"/>
		</xsl:when>

		<xsl:when test = "not(child::*[@KI='F']) and not(following-sibling::*[@KI='F'])">
			<img id="5" src="../../../images/tree/L.gif" onclick="JavaScript:Toggle('LP',this,'');"/>
		</xsl:when>

		<xsl:when test = "not(child::*[@KI='F']) and following-sibling::*[@KI='F']">
