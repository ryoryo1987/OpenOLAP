...
<xsl:element name="body">
	<xsl:attribute name="onload"><xsl:text>javaScript:Toggle('r',document.getElementById('root').firstChild.firstChild,'report_mng_tree_table.jsp?id=root')</xsl:text></xsl:attribute>
	...
	<div id="root" class="treeItem">
		<xsl:element name="img">
			...
			<xsl:attribute name="src">
				<xsl:choose>
					<xsl:when test="//BistroObjects[@flg='2']">../../../images/tree/report_top_p2.gif</xsl:when>
					<xsl:otherwise>../../../images/tree/report_top2.gif</xsl:otherwise>
				</xsl:choose>
			</xsl:attribute>
		<!--	<xsl:attribute name="src">../../../images/tree/report_top2.gif</xsl:attribute>-->
			<xsl:attribute name="onclick">javascript:setChangeFlg();Toggle('r',this,'report_mng_tree_table.jsp?id=root');</xsl:attribute>
			<xsl:attribute name="objkind"><xsl:value-of select="/*/@OBJKIND"/></xsl:attribute>
		</xsl:element>

		<xsl:element name="a">
			<xsl:attribute name="href">return false;</xsl:attribute>
			<xsl:attribute name="onclick">javascript:setChangeFlg();Toggle('r',this,'report_mng_tree_table.jsp?id=root'); return false;</xsl:attribute>
...
<xsl:template match="*" mode="tree-art">
		...
		<xsl:variable name="imgFileName">
			<xsl:choose>
				...
				<xsl:when test="(local-name()='category2') and @KI='R'">../../../images/file_p.gif</xsl:when>
				<xsl:when test="(local-name()='category2') and @KI='F'">../../../images/tree/folder_p2.gif</xsl:when>
			</xsl:choose>
		</xsl:variable>
	...
	<xsl:element name="div">
		...
		<xsl:element name="img">
			...
			<xsl:attribute name="onclick">javascript:setChangeFlg();Toggle('f',this,'<xsl:value-of select="$execFileName" />?id=<xsl:value-of select="@ID"/>');</xsl:attribute>
<!--		<xsl:attribute name="ondragstart">javascript:startDrag(); return false;</xsl:attribute> -->
		...
		<xsl:element name="a">
			<xsl:attribute name="href">return false;</xsl:attribute>
			<xsl:attribute name="onclick">javascript:setChangeFlg();Toggle('f',this,'<xsl:value-of select="$execFileName" />?id=<xsl:value-of select="@ID"/>'); return false;</xsl:attribute>
			<xsl:attribute name="id"><xsl:text /><xsl:value-of select="@ID" /><xsl:text /></xsl:attribute>
			<xsl:attribute name="name"><xsl:text /><xsl:value-of select="@ID" /><xsl:text /></xsl:attribute>
