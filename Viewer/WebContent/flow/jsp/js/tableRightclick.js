...
//var newWin;
	...
	// ツリーテーブル側
	if(menuList.url=="追加"){
	//	newWin = window.open("report_mng_member.jsp?key=-999","_blank","menubar=no,toolbar=no,width=300px,height=200px,resizable");
	//	newWin.moveTo(500,300);
		var arrArgObjs = [window.document,parent.frames[1]];
		newWin = showModalDialog("report_mng_member.jsp?key=-999",arrArgObjs,"status:false;dialogWidth:350px;dialogHeight:250px");

//		newWin = showModalDialog("rankingHie_member.jsp?mnu=add",window,"status:false;dialogWidth:380px;dialogHeight:250px");

	}else if(menuList.url=="名前の変更"){
		renameItem();
...
//***********Paste
	if(parent.navi_frm2.getPasteAble()){
	if(window.event.srcElement.tagName!='A'){
		strPaste=true;
	}
	}
...
var heightSize;
function createMenuOne(strMenuKind,i){
	var tempObj;
	var tempObj2;//Seprate用
	if(strMenuKind=='FIRSTMENU'){
		heightSize=0;
		tempObj = document.createElement('<DIV class=menuFront id=mainMenuFront style="VISIBILITY: hidden; WIDTH: 80px; HEIGHT: 10px;></DIV>');
	}else if(strMenuKind=='SEPARATE'){
		tempObj= document.createElement('<DIV class=itemSep style="WIDTH: 80px; TOP: '+heightSize+'px"></DIV>');
		...
		tempObj = document.createElement('<DIV class=menuBack id=mainMenuBack style="WIDTH: 80px; HEIGHT: '+heightSize-1+'px; BACKGROUND-COLOR: #c0c0c0"></DIV>');
	}else{
		tempObj=document.createElement('<DIV onClick="clickMenu(this);" class=item id=item'+i+' style="WIDTH: 80px; TOP: '+heightSize+'px" url="'+strMenuKind+'" target="_top" onMouseOver="mOver(this);" onMouseOut="mOut(this);"></DIV>');
...
function cutItem() {
	...
	// セレクションの保存
	parent.navi_frm2.setCutObjects(strSelectedKey);
}
...
//	newWin = window.open("report_mng_member.jsp?key=" + selectedKey,"_blank","menubar=no,toolbar=no,width=300px,height=200px,resizable");
//	newWin.moveTo(500,300);
	var arrArgObjs = [window.document,parent.frames[1]];
	newWin = showModalDialog("report_mng_member.jsp?key=" + selectedKey,arrArgObjs,"status:false;dialogWidth:350px;dialogHeight:250px");
...
function deleteItem(){
	...
	//左のTreeから取り除く
	parent.navi_frm2.rightObjDelete();


	document.SpreadForm.target="update_tree_frm";
	//階層カスタマイズ種別対応
	document.SpreadForm.action="../hidden/report_mng_tree_regist.jsp?opr=delete";
...
function pasteItem() {
	...
	var temp_obj;//
	temp_selection_text = parent.navi_frm2.getCutObjects();
	...
	var cutParentObject=parent.navi_frm2.getCutParentObject();//cutObjの所属元
	var preClickObj=parent.navi_frm2.getpreClickObj();//cutObjのpaste先
	...
	for(j=0;j<drop_id.length;j++){
		if(cutParentObject.lastChild.childNodes.length>i){//fukai 2004 02
			if(drop_id[j]==cutParentObject.lastChild.childNodes[i].id){
				...
				parent.navi_frm2.insertAdjustPosition(preClickObj,childFolder);
			}else{
				i++;
				j--;//必要？
			}
	...
	parent.navi_frm2.allLoop(cutParentObject,'ALL');

	parent.navi_frm2.allLoop(preClickObj,'ALL');

	//更新用hidden項目に、格納
	//更新されるべきレコード
	parent.navi_frm2.document.navi_form.child_record.value = temp_selection_text;
	//更新されるべき親のレコード
	//要考察
	parent.navi_frm2.document.navi_form.parent_record.value = preClickObj.id;
	parent.navi_frm2.document.navi_form.target="update_tree_frm";
	//階層カスタマイズ種別対応
	parent.navi_frm2.document.navi_form.action="../hidden/report_mng_tree_regist.jsp?opr=update";
	parent.navi_frm2.document.navi_form.submit();


//--右画面連携更新
	parent.right_frm.SpreadForm.target="_self"
//階層カスタマイズ種別対応
	parent.right_frm.SpreadForm.action="report_mng_tree_table.jsp?id=" + preClickObj.id;
	parent.right_frm.SpreadForm.submit();
//--右画面連携更新


	// 初期化して終了
	parent.navi_frm2.setCutObjects(null);
	return;
}

// ---- 下のメニューは使っていないが、ここだけコメントにしてはいけない、要注意
function createMenu(){
	frontMenu = document.createElement('<DIV class=menuFront id=mainMenuFront style="WIDTH: 50px; HEIGHT: 55px;"></DIV>');
	backMenu= document.createElement('<DIV class=menuBack id=mainMenuBack style="WIDTH: 52px; HEIGHT: 57px;"></DIV>');
	document.body.appendChild(frontMenu);
	document.body.appendChild(backMenu);
}