	...
	<xsl:if test="local-name()!='logical_model' or count(*//logical_column)!=0">
		<xsl:variable name="imgFileName">
			<xsl:choose>
				...
				<xsl:when test="local-name()='RDBModel'">../../../images/select.gif</xsl:when>
				<xsl:when test="local-name()='logical_model'">../../../images/gamen2.gif</xsl:when>
				<xsl:otherwise>../../../images/where_top.gif</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		...
		<xsl:element name="div">
			<xsl:attribute name="id"><xsl:text /><xsl:value-of select="@name" /><xsl:text /></xsl:attribute>
			...
			<xsl:element name="a">
				...
				<xsl:attribute name="id"><xsl:text />a_<xsl:value-of select="@name" /><xsl:text /></xsl:attribute>
...
<xsl:template match="*" mode="tree-art-where">
		<xsl:variable name="imgFileName">
			<xsl:choose>
				...
				<xsl:when test="./att1='Number'">../../../images/number.gif</xsl:when>
				<xsl:when test="./att1='Date'">../../../images/date.gif</xsl:when>
				<xsl:when test="local-name()='RDBModel'">../../../images/where_top.gif</xsl:when>
				<xsl:when test="local-name()='logical_condition'">../../../images/where.gif</xsl:when>
...
<xsl:template name="tree-hierarchy-select">
    <xsl:for-each select="ancestor::*">
        ...
        <xsl:choose>
			<xsl:when test="local-name()='select_clause'">
				<xsl:if test="../../following-sibling::*"><img id="H" src="../../../images/I.gif" ondragstart="return false;"/></xsl:if>
				<xsl:if test="not(../../following-sibling::*)"><img id="H" src="../../../images/blank.gif" ondragstart="return false;"/></xsl:if>
			</xsl:when>
			...
			<xsl:when test="local-name()='db_table'"><img id="H" src="../../../images/I.gif" ondragstart="return false;"/></xsl:when>
   ...
   <xsl:choose>                                                                                                      
		...
		<xsl:when test="local-name()='logical_model'"><!--Table論理名の表示-->
			<xsl:if test="../following-sibling::*"><img id="5" src="../../../images/Tminus.gif" onclick="JavaScript:Toggle('TP',this,'');" ondragstart="return false;"/></xsl:if>
			<xsl:if test="not(../following-sibling::*)"><img id="5" src="../../../images/Lminus.gif" onclick="JavaScript:Toggle('TP',this,'');" ondragstart="return false;"/></xsl:if>
		</xsl:when>
...
<xsl:template name="tree-hierarchy-where">
    <xsl:for-each select="ancestor::*">
        ...
        <xsl:choose>
			...
			<xsl:when test="local-name()='db_table'"><img id="H" src="../../../images/blank.gif" ondragstart="return false;"/></xsl:when>
   ...
   <xsl:choose>                                                                                                      
		   ...
		   <xsl:choose>                                                                                                      
			<xsl:when test = "child::* and not(../../../following-sibling::*)">
				...
				<xsl:if test="(following-sibling::*)">
					<img id="5" src="../../../images/T.gif" onclick="JavaScript:Toggle('LP',this,'');" ondragstart="return false;"/>
				</xsl:if>
			</xsl:when>
		...
		<xsl:when test="local-name()='RDBModel'"><!--抽出項目の表示-->
			<img id="5" src="../../../images/Lminus.gif" onclick="JavaScript:Toggle('TP',this,'');" ondragstart="return false;"/>
		</xsl:when>
