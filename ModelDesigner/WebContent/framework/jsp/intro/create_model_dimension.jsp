...
*/
		...
		//	strDimensions+=document.form_main.lst_right.options[j].text;
			strDimensions+=document.form_main.lst_right.options[j].value;
		}
		document.form_main.dimensions.value=strDimensions;
...
<div style="margin:10;height:250">
	...
	<table>
		<tr>
			<td>
				<span class="title">利用可能ディメンション：</span><br>
				<select name="lst_left" size="15" multiple style="width:225;margin-right:0">

<%
	int lstCnt=-1;
	String Sql="";
	Sql = " select m.dimension_seq,m.name,'' as selectedFlg from (select a.* from oo_dimension a,oo_user b where a.user_seq=b.user_seq and b.name='" + session.getValue("schema") + "') as m";
	if(!"".equals((String)session.getValue("ref_dimensions"))){
		Sql += " where m.dimension_seq not in (" + (String)session.getValue("ref_dimensions") + ")";
		Sql += " union";
		Sql += " select m.dimension_seq,m.name,'selected' as selectedFlg from (select a.* from oo_dimension a,oo_user b where a.user_seq=b.user_seq and b.name='" + session.getValue("schema") + "') as m";
		Sql += " where m.dimension_seq in (" + (String)session.getValue("ref_dimensions") + ")";
	}
	Sql += " ORDER BY 1"; 

	rs = stmt.executeQuery(Sql);
	while (rs.next()) {
		if(!"-1".equals(rs.getString("dimension_seq"))){
			lstCnt++;
			out.println("<option orderno='" + lstCnt + "' value='" + rs.getString("dimension_seq") + "' " + rs.getString("selectedFlg") + ">" + rs.getString("name") + "</option>");
		}
	}
	rs.close();
...
<br><br>
			...
			<td>
				<span class="title">選択ディメンション：</span><br>
...
<div style="text-align:right;padding-right:20px;padding-top:8px;margin-top:15px;border-top:1 solid #CCCCCC">
	...
	&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	<input type="button" value="" onclick="movePage('create_model_measure.jsp')" class="normal_back_mini" onMouseOver="className='over_back_mini'" onMouseDown="className='down_back_mini'" onMouseUp="className='up_back_mini'" onMouseOut="className='out_back_mini'">
...
<input type="hidden" id="fileName" name="fileName" value="create_model_dimension.jsp">
