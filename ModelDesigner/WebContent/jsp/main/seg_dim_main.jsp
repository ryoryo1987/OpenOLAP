...
<%
	...
	Sql = "SELECT d.name,d.comment,coalesce(d.dim_type,'1') AS dim_type,d.total_flg,coalesce(seg_datatype,'1') AS seg_datatype,coalesce(other_member_flg,'1') AS other_member_flg,d.sort_type,d.sort_order";
	...
	if(rs.next()){
		...
		strOtherMemberFlg = rs.getString("other_member_flg");
...
%>
				...
				document.all.div_hid.innerHTML += '<input type="hidden" name="hid_lv<%=rs.getString("level_seq")%>_longname" <%if(!"1".equals(rs.getString("level_no"))){out.print("mON=\"ロングネーム\"");}%> value="<%=rs.getString("long_name_col")%>">';
				document.all.div_hid.innerHTML += '<input type="hidden" name="hid_lv<%=rs.getString("level_seq")%>_shortname" <%if(!"1".equals(rs.getString("level_no"))){out.print("mON=\"ショートネーム\"");}%> value="<%=rs.getString("short_name_col")%>">';
				document.all.div_hid.innerHTML += '<input type="hidden" name="hid_lv<%=rs.getString("level_seq")%>_sortcol" <%if(!"1".equals(rs.getString("level_no"))){out.print("mON=\"ソートカラム\"");}%> value="<%=rs.getString("sort_col")%>">';
				...
				document.all.div_hid.innerHTML += '<input type="hidden" name="hid_lv<%=rs.getString("level_seq")%>_where_clause" <%if(!"1".equals(rs.getString("level_no"))){out.print("mON=\"WHERE句\"");}%> value="<%=ood.replace(ood.replace(rs.getString("where_clause"),"\\","\\\\"),"'","\\'")%>">';
				...
				if("<%=rs.getString("level_no")%>"=="1"){
					showDiv(<%=rs.getString("level_seq")%>,"seg","<%=rs.getString("name")%>",<%=rs.getString("x_point")%>,<%=rs.getString("y_point")%>);
				}else{
...
%>
		...
		function showDiv(levelSeq,type,objName,x_point,y_point){
			...
			if(type=="level"){
				...
				strHTML += '<div id="head_' + levelSeq + '" head_value="' + levelSeq + '" type="' + type + '" move="1" style="cursor:move;text-align:center;border:1 solid #C2AC74;border-bottom:3 double #C2AC74;color:#333333;padding:2 2 2 2;font-weight:bold;background-color:orangered;width:100;" basecolor="white" onclick="objectSelected(this);parent.showPpty(2,this);">セグメント</div>';
			}
			if(type=="level"){
				...
				strHTML += '<div id="body_' + levelSeq + '" head_value="' + levelSeq + '" type="' + type + '" move="2" style="cursor:crosshair;text-align:center;border:1 solid #C2AC74;padding:2 2 2 2;background:#ffeedd;width:100;height:30;OVERFLOW: hidden;TEXT-OVERFLOW: ellipsis;" onclick="objectSelected(parentNode.firstChild);parent.showPpty(2,this);">' + objName + '</div>';
			}
			...
			for(i=1;i<=max_num;i++){
				...
				if(chart.document.getElementById(div_id).objId!="0"){
					...
					for(lineNum=0;lineNum<chart.document.getElementById(div_id).lastChild.childNodes.length;lineNum++){
						...
						if((exeOrderNum==1)&&(chart.document.getElementById(div_id).firstChild.type=="seg")){
							chart.document.getElementById(div_id).firstChild.innerHTML="セグメント";
						}else{
		...
		function mappingCheck(){
			...
			for(var ob=0;ob<chart.document.all.length;ob++){
				if((chart.document.all(ob).tagName=="line")&&(chart.document.all(ob).id!="dashline")){
					...
					if(chart.document.getElementById(arr[0]).type!="seg"){
		...
		function showPpty(type,obj){
			...
			if(type==1){
				...
				document.getElementById("tbl_seg_level").style.display="none";
			...
			if(type==2){
				...
				document.getElementById("tbl_seg_level").style.display="block";
				document.getElementById("tbl_mapping").style.display="none";
				document.getElementById("tbl_none").style.display="none";
			}
			if(type==11){
		...
		function setData(){
			...
			}else if(document.getElementById("tbl_seg_level").style.display=="block"){
			//	chart.document.getElementById("body_" + document.form_main.hid_current_lvlseq.value).innerHTML=document.form_main.txt_seg_name.value;
				var lvlseq = document.form_main.hid_current_lvlseq.value;
				chart.document.getElementById("body_" + document.form_main.hid_current_lvlseq.value).innerHTML=document.form_main.txt_seg_name.value;
				document.form_main.elements["hid_lv" + lvlseq + "_name"].value=document.form_main.txt_seg_name.value;
				document.form_main.elements["hid_lv" + lvlseq + "_comment"].value=document.form_main.txt_seg_comment.value;
				document.form_main.elements["hid_lv" + lvlseq + "_table"].value=document.form_main.lst_seg_table.value;
				document.form_main.elements["hid_lv" + lvlseq + "_longname"].value="";
				document.form_main.elements["hid_lv" + lvlseq + "_shortname"].value="";
				document.form_main.elements["hid_lv" + lvlseq + "_sortcol"].value="";
				document.form_main.elements["hid_lv" + lvlseq + "_keycol1"].value=document.form_main.lst_seg_keycol.value;
				document.form_main.elements["hid_lv" + lvlseq + "_keycol2"].value="";
				document.form_main.elements["hid_lv" + lvlseq + "_keycol3"].value="";
				document.form_main.elements["hid_lv" + lvlseq + "_keycol4"].value="";
				document.form_main.elements["hid_lv" + lvlseq + "_keycol5"].value="";
				document.form_main.elements["hid_lv" + lvlseq + "_where_clause"].value="";
				document.form_main.elements["hid_lv" + lvlseq + "_x_point"].value=chart.document.getElementById("body_" + document.form_main.hid_current_lvlseq.value).parentNode.style.pixelLeft;
				document.form_main.elements["hid_lv" + lvlseq + "_y_point"].value=chart.document.getElementById("body_" + document.form_main.hid_current_lvlseq.value).parentNode.style.pixelTop;
		...
		function dispData(obj){
			if(document.getElementById("tbl_level").style.display=="block"){
				...
				var lvlseq = obj.head_value;
				document.form_main.txt_seg_name.value=document.form_main.elements["hid_lv" + lvlseq + "_name"].value;
				document.form_main.txt_seg_comment.value=document.form_main.elements["hid_lv" + lvlseq + "_comment"].value;
				document.form_main.lst_seg_table.options[selectedValue(document.form_main.lst_table,document.form_main.elements["hid_lv" + lvlseq + "_table"].value)].selected=true;
				createColumnList(document.form_main.elements["hid_lv" + lvlseq + "_table"].value,'lst_seg_keycol',document.form_main.elements["hid_lv" + lvlseq + "_keycol1"].value)
				document.form_main.hid_current_lvlseq.value=lvlseq;
			}else if(document.getElementById("tbl_mapping").style.display=="block"){
				...
				if(chart.document.getElementById("head_" + parLvlseq).type=="seg"){
				//	createColumnList(document.form_main.lst_seg_table.value,'lst_m_keycol1',document.form_main.elements["lst_seg_keycol"].value)
		...
		//テーブル切り替え時、カラムリストを変更
		function changeTabelList(obj,type){
			...
			if(type=="dim"){
				...
				document.form_main.lst_seg_keycol.options[0].selected=true;
			}
			...
			document.form_main.action = "../hidden/change_table.jsp?type=" + type + "&table_name=" + obj.value;
	...
	<table class="frame">
		...
		<tr>
			...
			<td class="main" style="padding-left:10px;padding-right:10px">
				...
				<div width="100%" align="right" style="margin-top:15px;margin-bottom:3px">
					<input type="button" value="" onclick="getNewSeq('level');setChangeFlg();" class="normal_level" onMouseOver="className='over_level'" onMouseDown="className='down_level'" onMouseUp="className='up_level'" onMouseOut="className='out_level'">
					<input type="button" value="" onclick="chart.del();setChangeFlg();" class="normal_delete_mini" onMouseOver="className='over_delete_mini'" onMouseDown="className='down_delete_mini'" onMouseUp="className='up_delete_mini'" onMouseOut="className='out_delete_mini'">
				</div>
				...
				<tr>
					...
					<td class="standard">
						<select name="lst_table" onchange="changeTabelList(this,'dim');setChangeFlg();">
							<option value="">---------------</option>
						<%
						Sql="";
...
-->
				...
				<table id="tbl_seg_level" style="display:none;width:100%">
				<tr>
					<th width="150" class="standard">
					レベル名
					</th>
					<td class="standard">
						<input type="text" name="txt_seg_name" value="" maxlength="30" size="60" onChange="setChangeFlg();">
					</td>
				</tr>
				<tr>
					<th width="150" class="standard">
					...
					<td class="standard">
						<input type="text" name="txt_seg_comment" value="" size="80" maxlength="250" onChange="setChangeFlg();">
					</td>
				</tr>
				<tr>
					<th width="150" class="standard">
					...
					<td class="standard">
						<select name="lst_seg_table" onchange="changeTabelList(this,'seg');setChangeFlg();">
							<option value="">---------------</option>
						<%
						Sql="";
						if("1".equals(strTableFlg)){
				...
				<tr>
					...
					<td class="standard">
						<select name="lst_seg_keycol" onchange="mappingCheck();setChangeFlg();">
							<option value="">---------------</option>
						</select>
					</td>
				</tr>
				<tr>
					...
					<td class="standard">
						<select name="lst_seg_data_type" onchange="mappingCheck();setChangeFlg();">
							<option value="1" <%if("1".equals(strSegDataType)){out.print("selected");}%>>数値</option>
							<option value="2" <%if("2".equals(strSegDataType)){out.print("selected");}%>>文字列</option>
						</select>
					</td>
				</tr>
				<tr>
					...
					<td class="standard">
						<input type="checkbox" name="chk_other_member_flg" value="1" mON="「その他」メンバーの作成" onChange="setChangeFlg();" <%if(strOtherMemberFlg.equals("1")){%>checked<%}%>>
					</td>
				</tr>
				</table>
